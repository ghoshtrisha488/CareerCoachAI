...
export function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (error) {
      setError(error);
      notify('Sign in failed: ' + error, 'error');
    } else {
    ...
    <div className="min-h-screen flex flex-col lg:flex-row bg-ink-50 dark:bg-ink-950">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-gradient-to-br from-brand-600 via-brand-700 to-brand-900 p-12 flex-col justify-between">
        ...
        <div className="relative text-white">
          <h2 className="font-display text-4xl font-bold leading-tight mb-4">Practice. Get feedback. Improve. Repeat.</h2>
          <p className="text-brand-100 text-lg max-w-md">AI-powered mock interviews tailored to your resume and target company. Real questions, real feedback, real results.</p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-sm">
          ...
          <h1 className="font-display text-2xl font-bold text-ink-900 dark:text-white mb-1">Welcome back</h1>
          <p className="text-sm text-ink-500 mb-6">Sign in to continue your interview prep</p>
          ...
          <p className="mt-6 text-center text-sm text-ink-500">
            ...
            <Link to="/signup" className="font-semibold text-brand-600 dark:text-brand-400 hover:underline">Sign up free</Link>
          </p>
        </motion.div>
      </div>
    </div>
